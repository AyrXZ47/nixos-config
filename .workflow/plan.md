...
| 2026-08-15 | **CORRECCIÓN de lo anterior**: el fix de nanoemoji está en el branch `master` de nixpkgs (desarrollo), pero NO en `nixos-unstable` (el canal que usa este flake, `flake.nix:5`). `nix flake update nixpkgs` trae la punta de nixos-unstable = `0e251e2` (2026-08-13) = la MISMA revisión rota; el rebuild vuelve a fallar con el mismo hash mismatch. El humano lo intentó y falló. Lock restaurado a `f13ff45` (la buena, cacheada, `nix build del toplevel` verificado OK) | Lección: verificar el fix contra el CANAL que usa el flake (`nixos-unstable`), no contra `master` de GitHub. Antes de volver a actualizar: comprobar que la punta de `nixos-unstable` ya pinea `sha256-FysyKC0...` (o una revisión posterior al 08-13) |
