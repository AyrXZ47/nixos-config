...
| 2026-08-14 | RECORRECCIÓN (datos del humano): la VRAM SÍ era causa — escritorio usa 0.9 GiB en reposo; con overhead 1 GiB ollama llega a **7.8 GiB totales → headroom real 0.2 GiB** → Hyprland cae a GTT. El overhead debe cubrir uso del escritorio (0.9) + margen. Config final: **overhead 2 GiB + CPU_THREADS 12** (ambas causas cubiertas a la vez, nunca probadas juntas antes; 27 t/s ya aceptado por el humano) | Medición humana: 0.9 GiB reposo → 7.8 GiB con ollama. La corrección anterior asumió solo CPU; la evidencia de VRAM del humano la refuta parcialmente — el fix correcto es AMBAS palancas |
| 2026-08-14 | FIN DE LA RULETA DE CONFIG — evidencia nvtop del humano: con kicad (789 MiB) + firefox (707) + mixxx (150) + shotcut (85) + mpvpaper el escritorio usa **~1.8 GiB de VRAM**, no 0.9 → con overhead 2 GiB el headroom real vuelve a ser ~0.2 GiB (las apps van POR ENCIMA de la reserva) → micro-freeze persiste aunque ollama respete todos los parámetros (24 t/s medidos: el coste de 2 GiB + 12 hilos). Reservar 2.75 GiB dejaría ~5.25 GiB → ~20 t/s (bajo la meta). **8 GiB de VRAM = pared física para un modelo de 23 GB + escritorio con apps pesadas** | Decisión: opción A recomendada = chat en `qwen3.5:9b` (37 t/s, 6.6 GB → 100% GPU, cero freezes) + `qwen3.6:35b-a3b-mtp-q4_K_M` solo para tareas largas de terminal. Config actual (overhead 2 GiB + 12 hilos) se mantiene como estabilidad-máxima |
| 2026-08-15 | Revertir OTRA VEZ `flake.lock` a `f13ff45` (2026-08-07); el humano había hecho `nix flake update nixpkgs` que re-trajo la rota `0e251e2` (2026-08-13) → hash mismatch de nanoemoji de nuevo. Revert aplicado por el planner (`6311717`), `nix flake check` pasa | Misma causa y misma solución que la decisión del 2026-08-14: la 08-13 sigue rota; `f13ff45` compila y está cacheada. Lección: NO `nix flake update nixpkgs` hasta que pase el problema de PyPI (re-subida de tarball) |
| 2026-08-15 | Los warnings `stdenv.isLinux/isDarwin is deprecated` NO son de este repo (grep limpio): vienen de Home Manager upstream (mods con `pkgs.stdenv.isLinux/isDarwin` en chromium, neovim, man, nushell, etc.). MASTER de HM ya migró a `stdenv.hostPlatform` (verificado en neovim/man/chromium) → se limpian solos actualizando el input `home-manager`. Son cosméticos, no bloquean nada | No vale la pena override local: los warnings son de evaluación, no afectan el build (`nix flake check` pasa igual). Cuando la red del ISP se estabilice, `nix flake update home-manager` los elimina sin tocar nixpkgs |
| 2026-08-15 | **SOLUCIÓN DEFINITIVA al update bloqueado**: overlay `unstableFixesOverlay` en `flake.nix` re-pinea el source de `nanoemoji` al hash real (`FysyKC0...`, el que usa master) en `nanoemoji`, `python313Packages.nanoemoji` y `python3Packages.nanoemoji`. Con esto el update a la punta de `nixos-unstable` (`0e251e2`) YA compila — `nix flake check` pasa, el source descarga OK. Patrón = PKGBUILD local de AUR: paquete roto upstream → parche local, no esperar al canal | JetBrains Mono construye vía `python313Packages.gftools → nanoemoji` (por eso el drv era "python3.13-nanoemoji"); el primer intento de overlay solo tocó el wrapper `nanoemoji` y no surtió efecto. El techo del parche: borrar cuando el canal pinne el hash bueno (redundante inofensivo mientras tanto) |
| 2026-08-15 | **Regla general GPU (humano)**: toda tarea que pueda completarse por GPU, va por GPU — «para algo compré la GPU». Aplicada a whisper-cpp: `vulkanSupport = true` (RX 7600 = backend Vulkan/RADV). Verificado: transcripción 2.8 s por GPU vs 27 s CPU (~10×), `ggml_vulkan: 0 = AMD Radeon RX 7600 (RADV NAVI33)` | De ahora en adelante, al añadir paquetes nuevos evaluar primero la ruta GPU (Vulkan/CUDA/ROCm según hardware) antes de la CPU por defecto |
| 2026-08-15 | Python global completo añadido (`modules/apps/python.nix`): python3 con numpy/scipy/pandas/matplotlib/ipython/pip/sympy + mplcyberpunk 0.7.6 empaquetado desde PyPI (hatchling) con parche para matplotlib 3.11 (`mpl.style.core.*` → `mpl.style.*`) | Confirmado que NO existía python global antes (solo el pythonEnv aislado de omnetpp). mplcyberpunk no está en nixpkgs → buildPythonPackage local. El parche es por el bug upstream de mplcyberpunk 0.7.6 con matplotlib ≥3.11; borrar cuando publiquen fix |
| 2026-08-15 | `subtitular` ahora usa `-of "''${FILE%.*}"`: el SRT sale con el nombre limpio del archivo origen, no el del wav temporal | Antes había que renombrar `X_temp.wav.srt` → `X.srt` a mano después de cada transcripción |
| 2026-08-15 | Shotcut roto para subtítulos por 2 causas raíz (verificadas en source v26.7.30): (1) `subtitlesdock.cpp:666` busca `ffmpeg` en `applicationDirPath()` (= el bin dir del propio shotcut) y nixpkgs no lo pone ahí → "ffmpeg not found. Cannot import subtitles."; (2) `settings.cpp:2049` busca `whisper-cli` igual → WhisperJob invoca al propio shotcut que rechaza los flags ("Unknown options"). Fix: wrapper `symlinkJoin` de shotcut con `ffmpeg_8` y `whisper-cpp.override { vulkanSupport = true; }` adyacentes en el mismo `bin/` (regla GPU cumplida: RX 7600 vía Vulkan verificado en el wrapper construido) | El patch de nixpkgs `fix-mlt-ffmpeg-path` solo cubre encode/melt jobs, NO el import de subtítulos ni whisper. ponytail: borrar el wrapper si nixpkgs algún día los junta |
| 2026-08-15 | One-shot `update-nixos` añadido a shell.nix: flake update → build seco → switch, con guarda de working tree limpio y host autodetectado | El humano pidió "un one shot para la próxima actualización" tras la saga del update roto; el build seco avisa antes de tocar el sistema si el canal trae algo roto |
| 2026-08-16 | **`geogebra6` ELIMINADO** de common-packages (upstream borró la URL del tarball 6-0-794 → 404; el humano no lo usa). Diagnóstico del "build de 10 horas": NO era la red (cache.nixos.org 12.9 MB/s, Cloudflare 18.5 MB/s medidos) — era un fetchurl con URL muerta que nix reintentaba con timeouts, disparando el estimador. MongoDB (mongod/mongos/mongosh) confirmado PRESENTE en el toplevel construido | El humano: «olvida geogebra, bórralo de todos lados, no lo uso, mongo sí es importante». Lección: cuando un build "tarda horas", medir la velocidad real del caché antes de culpar a la red; un 404 en fetchurl es la causa clásica |
| 2026-08-16 | **DNS declarativo completo** en networking.nix: `services.resolved` (systemd-resolved) con DoT a Cloudflare (`1.1.1.1#cloudflare-dns.com`) y Google (`8.8.8.8#dns.google`), DNSSEC=true, FallbackDNS=Telmex (200.33.146.1/148.1) solo si DoT muere, `Domains=~.` para rutear todo por el resolver local, `networking.networkmanager.dns = "systemd-resolved"`. Antes el DNS venía del DHCP (coincidía con el de Fedora por casualidad); ahora es fijo y cifrado en los 4 hosts | El humano pidió el setup de Fedora (Cloudflare + Google) pero declarativo y completo. OJO API: en este nixpkgs `services.resolved.extraConfig`/`dnssec`/`domains`/`fallbackDns` están renombrados a `services.resolved.settings.Resolve.*` (assertion si usas la API vieja) |
| 2026-08-16 | `hyprpicker` añadido a hyprland.nix: el color picker del Chroma Key de Shotcut "no hacía nada" porque xdg-desktop-portal-hyprland no encontraba slurp/hyprpicker para implementar PickColor (`[ERR] Neither slurp nor hyprpicker found`) | Diagnóstico con código fuente (colorpickeritem.cpp → portal Screenshot PickColor) + journals del portal. El picker de cualquier app que use el portal (no solo Shotcut) se arregla con esto |
| 2026-08-16 | Módulo `picker` de wayle (hyprpicker): icono `color-select-symbolic` junto al de wallpaper, click copia el color al portapapeles (`-a -f hex`) y notifica con el valor. Diagnosticado el "chroma key no hace nada" del humano: su clip tiene fondo MARRÓN oscuro (RGB ~102,56,55 muestreado en 15 puntos), no verde — el chroma key (bluescreen0r/select0r) no puede separar un fondo no-saturado de la piel; la herramienta correcta es Chroma Key Advanced con Similarity fino o máscara manual | El color guardado en el proyecto era #ff400c2a (magenta, con alpha 2a) — el eyedropper capturó mal o el LUT FilmBoost10 (aplicado después del chroma) cambia los colores que ve el usuario respecto al archivo fuente |
