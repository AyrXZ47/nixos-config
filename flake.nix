...
PYEOF
      ...
      # Caelestia: el CLI solo lista los esquemas que viven dentro de su propio
      # paquete (`scheme_data_dir` read-only en el store), no existe carpeta de
      # esquemas de usuario. Se inyecta `cyberpunk` (paleta de Wayle) con
      # postInstall para que `caelestia scheme set -n cyberpunk` funcione.
      caelestiaCyberpunkSchemeOverlay = final: prev: {
      ...
      mkHost = hostName: hostModules: nixpkgs.lib.nixosSystem {
        ...
        modules = [
          ...
          { nixpkgs.overlays = [ kdeconnectRemoteInputOverlay unstableFixesOverlay openrocketOverlay spiceLibraryOverlay bibataCursorOverlay caelestiaDedupeOverlay caelestiaCyberpunkSchemeOverlay ]; }
